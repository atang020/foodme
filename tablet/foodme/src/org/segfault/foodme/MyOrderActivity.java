...
public class MyOrderActivity extends Activity implements ActionBar.TabListener, OnItemClickListener{
	...
	private void toggleStatus(final int position)
	{
		...
		dialogBuilder.setPositiveButton("Yes", new DialogInterface.OnClickListener(){
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				TicketItem temp = new TicketItem(item.get(position).getOrderId(), item.get(position).getMenuItemId(), 
						item.get(position).getMenuItemIndex(), item.get(position).getQuantity(), item.get(position).getNotes(),
						item.get(position).getKitchenStatus(), item.get(position).getMenuItemName(), item.get(position).getPrice());
		...
		dialogBuilder.setNegativeButton("No", new DialogInterface.OnClickListener() {
			...
			@Override
			public void onClick(DialogInterface dialog, int which) {
				Toast waiterConfirm = Toast.makeText(getApplicationContext(),"Item status has NOT been changed..",Toast.LENGTH_SHORT);
